...
// Add click events to bar chart
setTimeout(() => {
  const bars = regularBarChart.querySelectorAll("rect");
  bars.forEach((bar, index) => {
    bar.style.cursor = "pointer";
    bar.addEventListener("click", (e) => {
      const label = barChart.config.data.labels[index];
      const value = barChart.config.data.datasets[0].data[index];
